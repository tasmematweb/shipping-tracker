...
function renderStoreProducts() {
    ...
    products.forEach(function(p) {
        ...
        const mainMedia = `<span class="product-icon">${p.icon || '📦'}</span>`;
        let sizesHtml = '';
        ...
        article.innerHTML = `
          <div class="product-image"><div class="main-media">${mainMedia}</div></div>
...
function renderAdminProducts() {
    ...
    products.forEach(function(p) {
        ...
        const sizeTags = p.sizes && p.sizes.length > 0 ? `<div class="size-tags">${p.sizes.map(s => `<span>${s}</span>`).join('')}</div>` : '';
        div.innerHTML = `
          <div class="item-info">
...
function renderAdminCategories() {
    ...
    categories.forEach(function(c) {
        ...
        div.className = 'admin-list-item';
        div.innerHTML = `
          <div class="item-info"><span>${c.icon || '🏷️'} ${c.name}</span></div>
...
function handleLogin() {
    ...
    // تسجيل دخول عادي (بدون رفع ملفات)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (video) {
        ...
        video.addEventListener('error', transitionToStore);
        video.play().catch(transitionToStore);
    } else {
        transitionToStore();
    }
});
